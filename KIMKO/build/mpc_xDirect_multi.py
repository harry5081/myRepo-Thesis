...
def functionTest1(v_ref, p_ref, v_init, p_init, v_input_begin):
    ...
    for i in range(window): # fill out all prediction state within a predic horz
        ...
        obj = obj + 1.5*i*(X[:,i] - P[2:4]).T @ Q @ (X[:,i] - P[2:4]) + V_INPUT_MATRIX[i] **2
        #obj = obj + 10*(X[:,i+1] - P[2:4]).T @ Q @ (X[:,i+1] - P[2:4])+V_INPUT_MATRIX[i] **2
    ...
    #args["lbg"] = -float('inf')
    #args["ubg"] = float('inf')
    args["lbg"] = vertcat(np.zeros(2*(window+1)),-150*np.ones(window))
    args["ubg"] = vertcat(np.zeros(2*(window+1)),150*np.ones(window))
