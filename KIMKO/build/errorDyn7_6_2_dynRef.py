...
p_ref=np.array([[8,191,3.12],[4,200,3.15],[-1,198,3.19]]) # give phi 90 check radius degree!!!!!!!!!!!!
v_ref=np.array([[50,0,0.25],[50,0,0.25],[50,0,0.25]])   # vp, 0, wp
...
for i in range(0,window):
    ...
    obj=obj+(err_next.T @ Q @ err_next)# + 7*demand_cur[3]**2# + (demand_cur[3]-ref_current[3])**2     #5*demand_cur[3]**2#+ 10*(demand_cur[5]-ref_current[5])**2
...
args["x0"] = vertcat(reshape(d0,6*(window),1),repmat(np.zeros(3),(window+1),1)) 
#args["x0"] = vertcat(repmat(np.zeros(3),(window+1),1), reshape(d0,6*(window),1)) #initial guess of demand state
