...
fspeed_vel = sqrt((model[0]+0.00001)**2+(model[2]+0.00001)**2)
#fs = Function('fs',[states,controls],[fspeed_vel])
fspeed_angle = np.arctan2(model[0]+0.001, model[2]+0.0000001) * 180 / np.pi + model[5]  ### still need unwrap
...
for i in range(window): # fill out all prediction state within a predic horz
    ...
    #obj = obj + (X[:,i+1] - P[6:12]).T @ Q @ (X[:,i+1] - P[6:12]) + (FSPEED_STATE[:,i]-10)**2
    obj = obj + (FSPEED_STATE[:,i][0]-fs_ref[0])**2 + 0.0001*(FSPEED_STATE[:,i][1]-fs_ref[1])**2
    #obj = obj + (FSPEED_STATE[:,i]-fs_ref).T @ R3 @(FSPEED_STATE[:,i]-fs_ref)
    
    #+ (V_INPUT_MATRIX[:,i].T @ R @ V_INPUT_MATRIX[:,i])#+ (control_diff.T @ R2 @ control_diff)
...
fspeed_ff = Function('fspeed_ff',[U,X],[FSPEED_STATE])
...
FSPEED = fspeed_ff(u,state)
fs_predichorz_update = np.zeros((1,2,window))
fs_predichorz_update[0,:,:] = FSPEED
...
for i in range(1):
    for j in range(window):
        ...
        print("fspeed = ",fs_predichorz_update[i,:,j])
        print("")
        print("")
