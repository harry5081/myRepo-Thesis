...
#def functionTest():
    ...
    ipopt_options={}
    ipopt_options["max_iter"] = 10#100
    ipopt_options["print_level"] = 0
    ipopt_options["acceptable_tol"] = 1e-1#1e-6#
    ipopt_options["acceptable_obj_change_tol"] = 1e-1#1e-6
    ...
    args["lbg"] = vertcat(-1*np.ones(2*(window+1)),-float('inf')*np.ones(window))
    args["ubg"] = vertcat(np.ones(2*(window+1)),float('inf')*np.ones(window))
