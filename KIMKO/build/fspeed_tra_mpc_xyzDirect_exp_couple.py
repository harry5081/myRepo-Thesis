...
fspeed_angle = np.arctan2(vx, vy) * 180 / np.pi
fspeed_states = vertcat(fspeed_vel, fspeed_angle)
fs = Function('fs',[vx,vy],[fspeed_states])
...
for i in range(window): # fill out all prediction state within a predic horz
    ...
    FSPEED_STATE[:,i]= fs(state_current[0],state_current[2])
    ...
    obj = ( obj + 10*(X[:,i+1] - ref_current).T @ Q @ (X[:,i+1] - ref_current) +
            V_INPUT_MATRIX[:,i].T @ V_INPUT_MATRIX[:,i]+ 3*v_input_dff.T @ v_input_dff )# + (control_diff.T @ R2 @control_diff)
